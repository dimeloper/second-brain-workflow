...
description: >-
  ...
  dev-standards rules, add a thin always-apply onboarding rule, point at
...
- Standards (source of truth): `~/dev-standards`
- Rules sync: `~/dev-standards/scripts/sync-rules.sh`
- Skills sync: `~/dev-standards/scripts/sync-skills.sh`
- Skills source: `~/dev-standards/cursor-skills/{workflow,...}/`
- Vault practices: `$DEV_STANDARDS_VAULT/practices/` (default `~/vaults/second-brain`)
...
```bash
~/dev-standards/scripts/render.py "<TARGET>"          # all configured targets
~/dev-standards/scripts/render.py "<TARGET>" --targets cursor,agents
```

One canonical rule set (`rules/*.md`, wherever `DEV_STANDARDS_RULES_DIR` resolves to) renders per agent:
...
Everything is a real file carrying a `generated by dev-standards@<sha>`
...
- Start from `AGENTS.md` (synced from dev-standards). Architecture/commands: `CLAUDE.md` if present.
- Shared stack rules: vendored copies under `.cursor/rules/` — generated by `~/dev-standards/scripts/render.py`; edit the source rules and re-render, not here.
...
(`$DEV_STANDARDS_VAULT/YYYY-MM-DD.md`) — e.g. “Onboarded `<repo>` into
dev-standards”. Daily notes need **no approval** (see
...
Libs, docs, `dev-standards`, vault — no product MCP. Say “MCP skipped” in Done.
