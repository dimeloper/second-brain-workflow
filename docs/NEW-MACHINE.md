...
It is **appended to `AGENTS.md`** under its own description, with its own
provenance header, and that is the only place it is written:
...
overwritten. The run says so when it happens, and names the directories the
rules landed in instead.
