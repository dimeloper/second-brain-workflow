...
### ...and a rule with no `paths:`

An **always-on** rule — no `paths:` at all — is delivered differently, and this
is the part v0.20.0 changed. `rules/verify-integrations.md`:

```markdown
---
description: Before calling an integration done, run it once against the real external system
---

- Run it for real against that system at least once. A local suite passing
  against fixtures is not evidence.
```

For Cursor it still gets its own file, `.cursor/rules/verify-integrations.mdc`,
with `alwaysApply: true` — Cursor does not read `AGENTS.md`, so there is nothing
to fold into.

For `agents` and `claude-code` it is **appended to `AGENTS.md`** under its own
description, with its own provenance header:

```markdown
<!-- generated by second-brain-workflow@70b2189 (v0.20.0) from rules/verify-integrations.md -->

## Before calling an integration done, run it once against the real external system

- Run it for real against that system at least once. A local suite passing
  against fixtures is not evidence.
```

and **no `.claude/rules/verify-integrations.md` is written**. Claude Code reads
it through `CLAUDE.md`'s `@AGENTS.md` import; a per-rule file as well would load
the same text twice and charge the budget for both.

Two cases fall back to that per-rule file, because there is nothing to fold
into: rendering `claude-code` **without** `agents`, and a target repo whose
`AGENTS.md` is hand-written and therefore never overwritten. The run says so
when it happens.
...
`tests/test-troubleshooting.sh` checks that the messages this table **quotes**
