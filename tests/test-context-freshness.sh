...
# --- an engine write is not product movement --------------------------------
#
# The defect this closes. Tier 1 lists CLAUDE.md and AGENTS.md, and in an
# onboarded repo those are frequently this engine's own output — so onboarding a
# repo, or any later re-render, moved a "product doc" and marked that repo's
# whole context/ stale with no product fact changed. On 2026-09-22 `d427cbb`
# (`chore: onboard the app into second-brain-workflow as shared`) did exactly
# that to all three of babypath-app's context files.
#
# Two shapes, because the engine writes two ways: a whole file it owns, marked;
# and a single `@AGENTS.md` import prepended to a CLAUDE.md the repo owns, which
# carries no marker because the file is genuinely hand-written.

MARKER_TEXT="generated by second-brain-workflow@"

# A repo whose last *product* commit predates the context, and whose only later
# commit is an engine write. Before the fix this reported STALE.
r="${SANDBOX}/repos/onboarded-app"
mkdir -p "${r}"
git -C "${r}" init -q 2>/dev/null
git -C "${r}" remote add origin "git@github.com:acme/onboarded-app.git"
printf '# Product\n\nWhat it is for.\n' > "${r}/PRODUCT.md"
printf '# Team notes\n\nHow we work.\n' > "${r}/CLAUDE.md"
git -C "${r}" add -A
GIT_AUTHOR_DATE="2026-05-20T12:00:00" GIT_COMMITTER_DATE="2026-05-20T12:00:00" \
  git -C "${r}" -c user.email=t@example.com -c user.name=t commit -qm "product doc"
# ...then onboarding, later, touching only the import line.
printf '@AGENTS.md\n\n# Team notes\n\nHow we work.\n' > "${r}/CLAUDE.md"
git -C "${r}" add -A
GIT_AUTHOR_DATE="2026-07-01T12:00:00" GIT_COMMITTER_DATE="2026-07-01T12:00:00" \
  git -C "${r}" -c user.email=t@example.com -c user.name=t commit -qm "chore: onboard"

make_context onboarded "audience" "2026-06-01" '["onboarded-app"]'
out_onb="$("${CF}" --vault "${VAULT}" --project onboarded 2>&1)"
rc_onb=$?
assert_exit 0 "${rc_onb}" "an engine-only commit does not make a context stale"

TESTS_RUN=$((TESTS_RUN + 1))
case "${out_onb}" in
  *STALE*) fail "the onboarding commit is not read as product movement" "${out_onb}" ;;
  *) pass "the onboarding commit is not read as product movement" ;;
esac

# The date reported is the last *product* commit, not the engine one — so a
# reader can check the claim rather than take the verdict on trust.
TESTS_RUN=$((TESTS_RUN + 1))
case "${out_onb}" in
  *2026-05-20*) pass "and the date shown is the last real product commit" ;;
  *) fail "and the date shown is the last real product commit" "${out_onb}" ;;
esac

# A real product commit after the same onboarding still reports STALE: the fix
# must not swallow movement that happened to land behind an engine write.
printf '# Product\n\nWhat it is for, revised.\n' > "${r}/PRODUCT.md"
git -C "${r}" add -A
GIT_AUTHOR_DATE="2026-08-01T12:00:00" GIT_COMMITTER_DATE="2026-08-01T12:00:00" \
  git -C "${r}" -c user.email=t@example.com -c user.name=t commit -qm "feat: revise"
out_after="$("${CF}" --vault "${VAULT}" --project onboarded 2>&1)"
TESTS_RUN=$((TESTS_RUN + 1))
case "${out_after}" in
  *STALE*2026-08-01*) pass "a real commit behind an engine write is still found" ;;
  *) fail "a real commit behind an engine write is still found" "${out_after}" ;;
esac
rm -rf "${VAULT}/projects/onboarded"

# --- a generated file is not a tier source at all ---------------------------
r2="${SANDBOX}/repos/generated-app"
mkdir -p "${r2}"
git -C "${r2}" init -q 2>/dev/null
git -C "${r2}" remote add origin "git@github.com:acme/generated-app.git"
printf '# Product\n\nWhat it is for.\n' > "${r2}/PRODUCT.md"
git -C "${r2}" add -A
GIT_AUTHOR_DATE="2026-05-20T12:00:00" GIT_COMMITTER_DATE="2026-05-20T12:00:00" \
  git -C "${r2}" -c user.email=t@example.com -c user.name=t commit -qm "product doc"
printf '<!-- %sabc123 (v0.59.0) — do not edit here -->\n\n# Rules\n' "${MARKER_TEXT}" \
  > "${r2}/AGENTS.md"
git -C "${r2}" add -A
GIT_AUTHOR_DATE="2026-07-01T12:00:00" GIT_COMMITTER_DATE="2026-07-01T12:00:00" \
  git -C "${r2}" -c user.email=t@example.com -c user.name=t commit -qm "render"

make_context generated "audience" "2026-06-01" '["generated-app"]'
out_gen="$("${CF}" --vault "${VAULT}" --project generated 2>&1)"
assert_exit 0 $? "a marked AGENTS.md is not counted as a product doc"
TESTS_RUN=$((TESTS_RUN + 1))
case "${out_gen}" in
  *STALE*) fail "a generated AGENTS.md does not age a context file" "${out_gen}" ;;
  *) pass "a generated AGENTS.md does not age a context file" ;;
esac
rm -rf "${VAULT}/projects/generated"
