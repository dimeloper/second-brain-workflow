...
from lib.provenance import MARKER
import os
...
# The canonical definition is lib/provenance.MARKER; this alias keeps the
# name this module's callers already use. lib/registry.sh still carries its
# own copy because it is shell, and test-repo-registry.sh guards that one.
# tests/test-repo-registry.sh asserts all three are the same string: three
# copies of it is three chances for a check to stop matching anything and say
# nothing about it.
RENDER_MARKER = MARKER

# The always-on files a render may write. Checked only for the marker — a
# hand-written AGENTS.md is not evidence of a render, which is the same rule
...
def rendered(repo):
    """Does `repo` still carry output a render wrote? The Python mirror of
    ...
    for name in MARKER_FILES:
        ...
        if not candidate.is_file():
            continue
        try:
            if RENDER_MARKER in candidate.read_text(encoding="utf-8", errors="replace"):
                return True
        except OSError:
            continue
    return False
...
def write_entries(path, entries, warn=None, describe=None):
    """Replace `path` with `entries` ({repo: fields}), atomically. -> bool.

    Temp file plus replace, in the same directory: a failure partway through
    must leave the previous list intact rather than truncate it. Sorted on
    write, so the same repo recorded twice leaves one line and a diff of the
    file stays readable.

    Never raises. `describe(path, reason)` composes the warning, because what a
    failed write *means* differs per caller — a render that could not record
    itself and a decline that could not be remembered leave different states
    behind, and a generic "could not write" would say neither.

    Shared with lib/onboarding.py, whose declined list is the same format in a
    sibling file: the atomicity and the sorting are the parts that are easy to
    ...
    try:
        ...
        if warn:
            ...
            warn(describe(path, reason) if describe
                 else f"could not write {path}: {reason}")
        return False
...
def register(repo, mode=None, warn=None):
    ...
    return write_entries(
        path, entries, warn,
        describe=lambda p, reason: (
            f"rendered, but could not record this repo in {p}: {reason}. "
            "Until a render succeeds in writing it, doctor reports the "
            "onboarded repo set as undetermined."
        ),
    )
...
def forget(repo, warn=None):
    ...
    return write_entries(
        path, entries, warn,
        describe=lambda p, reason: (
            f"removed the rendered files, but could not update {p}: {reason}. "
            "The registry still names this repo, so doctor will report it as "
