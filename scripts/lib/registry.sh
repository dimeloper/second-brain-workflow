...
# written on every successful render regardless of RENDER_TARGETS, so a repo
# carrying neither marker file (a cursor-only render against an engine with no
# AGENTS.md of its own) is not mistaken for a stale entry. The marker files are
# the fallback for a repo rendered by an engine old enough not to have written
# it.
