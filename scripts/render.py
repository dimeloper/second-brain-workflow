...
from lib.provenance import MARKER, is_generated  # noqa: E402
...
# Root files this engine used to write and no longer does. They are pruned on
# the next render, under exactly the guards the per-rule prune uses — ours by
# marker, and never a symlink the repo made for itself.
#
# That second guard is the whole reason this is a named list rather than an
# unlink. A repo can reasonably commit `AGENTS.md -> CLAUDE.md`, and a prune
# that resolved the link would delete the repo's own file through it. The write
# path learned this in v0.55.0 (see foreign_symlink); a prune added later
# without the same test would reintroduce it from the other side, on a path
# nobody is looking at because the file is *supposed* to disappear.
PRUNED_ROOT = ("CLAUDE.md",)
...
# A generated CLAUDE.md used to sit here, holding a provenance comment and an
# `@AGENTS.md` line, because Claude Code read CLAUDE.md and not AGENTS.md. Since
# 2.1.277 it reads a root AGENTS.md in a project with no CLAUDE.md, so the stub
# forwarded to a file the agent would have read on its own — and, by existing at
# all, was the only reason it had to. `make doctor` reports whether the machine
# in front of you is new enough; PRUNED_ROOT below removes the ones already
# written.
...
def warn_claude_md_shadows_agents(repo, targets, fold):
    """A hand-written CLAUDE.md hides the AGENTS.md we just wrote. Say so.

    This used to be a line under `skip (hand-written, not ours): CLAUDE.md`,
    which worked only because we rendered a CLAUDE.md of our own and therefore
    had a reason to look at the path. Dropping that stub took the warning with
    it — and made it matter more, not less: with no stub, a repo whose
    CLAUDE.md is its own and does not import AGENTS.md is a repo where Claude
    Code reads neither the fold nor, because CLAUDE.md exists at all, the
    native AGENTS.md fallback. The always-on set reaches it nowhere, silently.
    So the check is now its own thing, keyed on the repo's shape rather than on
    what we happened to write.

    Silent in the three cases where the advice would be wrong, each a real
    onboarded repo:

      - CLAUDE.md already imports AGENTS.md. Advice a reader has acted on is
        otherwise indistinguishable, next run, from advice they ignored.
      - AGENTS.md is not ours (`fold` false), so the always-on rules are per-
        rule files under .claude/rules/ and Claude Code has them already.
        Importing a hand-written AGENTS.md would pick up nothing.
      - AGENTS.md is a symlink to CLAUDE.md, which `fold` is already false for:
        following the advice would make the file import itself.
    """
    if "claude-code" not in targets or not fold:
        return
    dest = repo / "CLAUDE.md"
    if not dest.is_file() or is_generated(dest):
        return
    try:
    ...
    if "@AGENTS.md" in body:
        return
    print("note: CLAUDE.md here is hand-written, and Claude Code reads it "
          "instead of\n      AGENTS.md. Add `@AGENTS.md` at its top, or the "
          "always-on rules reach\n      Claude Code nowhere in this repo.",
          file=sys.stderr)
...
def main():
    ...
    # Root files we used to write. Same three questions as above and one more:
    # a link the repo owns is not ours to remove, whatever it resolves to.
    for rel in PRUNED_ROOT:
        existing = repo / rel
        if not existing.exists() and not existing.is_symlink():
            continue
        if foreign_symlink(repo, existing) or not is_generated(existing):
            continue                    # the repo's own file, or its own link
        if mode == "check":
            print(f"  DRIFT: stale generated file {rel}")
            drift = 1
        elif mode == "dry-run":
    ...
    warn_claude_md_shadows_agents(repo, targets, fold)
