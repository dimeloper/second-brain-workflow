...
last-reviewed: {{DATE}}
repos: []
tags: [obsidian, daily-notes, review]
---

# Record declined vault candidates, not just approved ones

**Rule:** When a `## Vault candidates` bullet is discussed and not written — rejected, deferred, or superseded — log it under `## Vault writes (declined)` in the same daily note with a one-line reason. A deferred promotion counts: say what would unblock it.
